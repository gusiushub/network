...
class Form extends Model
{
    ...
    public function regValidate()
    {
        ...
        $count = count($this->queryRows("SELECT login FROM users WHERE email='".htmlspecialchars($_POST['E-mail'])."'"));
        $login = $this->queryRow("SELECT login FROM users WHERE login='".htmlspecialchars($_POST['login'])."'");
        if($count==0){
            if (isset(
                        $_POST['first_name'],
                        $_POST['last_name'] ,
